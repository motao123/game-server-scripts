...
脚本支持两种服务器类型，部署时会让你选择：

| 特性 | Paper（推荐） | Vanilla（原版） |
|------|--------------|----------------|
| 性能 | 优化过，TPS 更稳定 | 原版性能，大型服务器吃力 |
| 插件支持 | 支持 Bukkit/Spigot/Paper 插件 | 不支持插件 |
| Bug 修复 | 修复了大量原版 Bug | 原版 Bug 保留 |
| 更新速度 | 紧跟原版更新，通常只慢几天 | 第一时间更新 |
| 适用场景 | 多人服务器、需要插件 | 纯原版体验、单人开服 |
...
如果你想玩 **Mod**（如工业、暮色森林、JEI 等），Paper 服务器不支持 Mod，需要换成 **Forge** 或 **Fabric**。
...
```bash
# 下载 Fabric 服务端安装器
wget https://meta.fabricmc.net/v2/versions/loader/1.21.4/0.16.14/1.0.1/server/jar -O fabric-server.jar

# 或用安装器
wget https://maven.fabricmc.net/net/fabricmc/fabric-installer/1.0.1/fabric-installer-1.0.1.jar
java -jar fabric-installer-1.0.1.jar server -dir /opt/minecraft -downloadMinecraft
```
...
```bash
# 下载 Forge 安装器
wget https://maven.minecraftforge.net/net/minecraftforge/forge/1.21.4-54.0.16/forge-1.21.4-54.0.16-installer.jar

# 安装服务端
java -jar forge-*-installer.jar --installServer /opt/minecraft
```
