...
function GenericInstances({ instances, post }: { instances: any[]; post: (p: string, b: any, ok: string) => void }) {
  ...
  return <Card title="通用实例" extra={<span>支持自定义工作目录、启动命令、停止命令</span>}>
    ...
    <Table rowKey="id" dataSource={instances} pagination={false} className="section-card" columns={[
      { title: '名称', dataIndex: 'name' }, { title: '类型', dataIndex: 'instanceType' }, { title: '状态', dataIndex: 'status', render: (s: string, r: any) => <Space direction="vertical" size={2}><Tag color={s === 'running' ? 'green' : s === 'error' ? 'red' : s === 'starting' || s === 'stopping' ? 'blue' : 'default'}>{s}</Tag>{s === 'error' && r.lastError && <span style={{ color: '#C10015', fontSize: 12, maxWidth: 220 }}>{r.lastError}</span>}</Space> }, { title: 'PID', dataIndex: 'pid', width: 80, render: (p: number) => p || '-' },
      { title: '操作', render: (_, r: any) => <Space><Button onClick={() => startInstance(r)} disabled={r.status === 'running'} loading={starting === r.id}>启动</Button><Button onClick={() => stopInstance(r)} disabled={r.status !== 'running'} loading={stopping === r.id}>停止</Button><Button onClick={() => post('/api/instances/restart', { id: r.id }, '已重启')} disabled={r.status !== 'running'}>重启</Button><Button onClick={() => openConsole(r)} disabled={r.status !== 'running' && !r.lastStarted && r.status !== 'error'}>{r.status === 'error' ? '查看日志' : '控制台'}</Button><Button onClick={() => openEdit(r)}>编辑</Button><Button danger onClick={() => delInstance(r)}>删除</Button></Space> }
    ]} />
